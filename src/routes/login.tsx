      ...
      <div className="w-full max-w-sm space-y-4">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-fg-muted">
            เข้าสู่ระบบเพื่อใช้งานห้องควบคุม
          </p>
        </div>
          ...
          <div className="space-y-3">
            {GROK_PROVIDERS.map((p) => (
              <button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-[var(--radius-md)] border border-border bg-surface px-4 py-3 text-sm font-medium transition-colors hover:bg-subtle"
              >
                {p.label === "Google" && (
        ...
        <p className="text-center text-xs text-fg-subtle">
          ใช้ระบบ Grok Auth Broker (OAuth จริงผ่าน Google / X)
        </p>
      </div>
    </main>
  );
